...
async function main() {
  ...
  var baseCredentialText =
    "{Credential_ID:$ID; Issuer:$ISSUER; Subject:$SUBJECT;}";
  ...
  baseCredentialText = baseCredentialText.replace("$ISSUER", puK);
  ...
  credential_1_Text = baseCredentialText.replace(
    "$SCHOOL",
    newEntityEpicWallet.identity_ExtPublicKey
  );

  credential_1_Text = credential_1_Text.replace(
    "$ID",
    "87341868-10b0-4a35-971c-b26974b89cb3"
  );
  credential_2_Text = credential_1_Text.replace(
    "$ID",
    "1e0ca9b7-4a20-493a-9f4f-b253febc8379"
  );
  credential_3_Text = credential_1_Text.replace(
    "$ID",
  ...
  newSPWallet.setMnemonic(
    "hunt angle stage hurt promote daring burger loan ignore kind reform dry"
  );
  // mZR_der, SSSSSW_der, MTN_der
  ...
  newSPWallet.setIdentityDerivation(
    "m/1037171/415581744",
    "/1687453124/2141050260/1229666004/344302187/2118467628/304801079",
    "/131071/0407/10011001"
  );

  // Exchange between user and service provider
  newSPWallet.addCPlusDerivation("User");
  ...
  connect_to_service_provider =
    newUserEpicWallet.getBPlusDerivation("ServiceProvider");
  user_sp_relationship_public_key =
    connect_to_service_provider.data.own_extendedPublicKey;
  ...
  let login_puK = newSPWallet.getPurposePublicKey(purpose);
  purpose = "credencialIssuance_extPublicKey";
  let cred_puK = newSPWallet.getPurposePublicKey(purpose);
  purpose = "presentations_extPublicKey";
  let pres_puK = newSPWallet.getPurposePublicKey(purpose);
  ...
  newSPWallet.updateCPlusDerivationExtendedKeys(
    "User",
    user_sp_relationship_public_key
  );
  ...
  newUserEpicWallet.setPresentationDerivation(
    "ServiceProvider",
    "7c3d4c06-891d-4bdf-aa72-f702aa2e66bc",
    "47807"
  );

  subjectPublicKey = newUserEpicWallet.getPresentationExtendedPublicKey(
    "ServiceProvider",
    "7c3d4c06-891d-4bdf-aa72-f702aa2e66bc"
  );

  credential_setSignature = await newUserEpicWallet.signPresentation(
    "ServiceProvider",
    "7c3d4c06-891d-4bdf-aa72-f702aa2e66bc",
    credential_set
  );
  ...
  cred1_der = newUserEpicWallet.getCredentialDerivation(
    "AcmeAcademy",
    "87341868-10b0-4a35-971c-b26974b89cb3"
  );
  cred2_der = newUserEpicWallet.getCredentialDerivation(
    "AcmeAcademy",
    "1e0ca9b7-4a20-493a-9f4f-b253febc8379"
  );
  cred3_der = newUserEpicWallet.getCredentialDerivation(
    "AcmeAcademy",
    "aed59aca-d62d-4e0a-a576-c2b34a8e6d8a"
  );
  ...
  presentation_derivation = newUserEpicWallet.getPresentationDerivation(
    "ServiceProvider",
    "7c3d4c06-891d-4bdf-aa72-f702aa2e66bc"
  );
  ...
  if (newSPWallet.verifyPresentationSignature(
    "User",
    presentation_derivation,
    credential_set,
    credential_setSignature
  )) {
    console.log("CORRECT presentation signature");
  }
  else {
    console.log("INVALID presentation signature");
  }
