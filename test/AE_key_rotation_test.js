...
async function main() {
  ...
  console.log(
    "3rd test: CASE 2: Step 1. Assign a dummy credential and presentation to user"
  );
  ...
  old_user_entity_relationship_public_key =
    old_connect_to_entity.data.own_extendedPublicKey;

  // UserWallet changes BPlusDerivation for that entity
  oldDerivation = newUserEpicWallet.renewBPlusDerivation(
    "AcmeAcademy",
    "14876572"
  );
  ...
  user_entity_relationship_public_key =
    connect_to_entity.data.own_extendedPublicKey;
  ...
  let oldCreds = newUserEpicWallet.getOldCredentials(
    "AcmeAcademy",
    oldDerivation
  );
  let oldPres = newUserEpicWallet.getOldPresentations(
    "AcmeAcademy",
    oldDerivation
  );
  ...
  newUserEpicWallet.generateNewIdentity(
